...
function renderMessages(): void {
  ...
  chat.messages.forEach((msg) =>
    msgContainer.appendChild(createMessageEl(msg)),
  );
  scrollToBottom();
}
...
function createMessageEl(msg: Message): HTMLElement {
  ...
  if (msg.role === "assistant" && msg.sources?.length) {
    ...
    heading.textContent = `Top ${Math.min(msg.sources.length, 3)} relevant passages`;
      ...
      card.innerHTML = `
        <summary>
          ...
          <span style="display:flex;align-items:center;gap:6px;">
            <span class="score-badge">${(src.similarity * 100).toFixed(1)}% similarity</span>
...
function distanceToSimilarity(distance: number): number {
  ...
  return Math.max(0, Math.min(1, 1 / (1 + safeDistance)));
}
  ...
  try {
    ...
    const assistantMsg: Message = {
      ...
      sources: data.sources?.map((s: string, i: number) => ({
        text: s,
        similarity: distanceToSimilarity(Number(data.distances?.[i] ?? 0)),
        source: `Top result ${i + 1}`,
      })),
    };
